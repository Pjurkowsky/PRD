...
@router.post("/address")
def post_address(address: AddressSchema):
...
@router.post("/person")
def post_person(person: PersonSchema):
...
def post_applicant(applicant: ApplicantSchema):
...
def post_application_verification(application_to_verify: ApplicationVerificationSchema, current_user: str = Depends(get_current_user)):
    query = "SELECT * FROM User WHERE username=%s"
    cursor.execute(query, (current_user,))
    user = cursor.fetchone()
...
def post_application(application: ApplicationSchema):
    query = "INSERT INTO Application (application_type_id, applicant_id) VALUES (%s, %s)"
